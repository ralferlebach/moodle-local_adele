...
class catquiz implements course_completion {
                ...
                if (
                    ...
                    if ($test != null) {
                        ...
                        '" target="_blank">' . format_string($test->name, true,
                            ['context' => \context_system::instance(), 'filter' => false]) . '</a>';
                    } else {
    ...
    private function get_attempts_information($attemptids) {
        ...
        [$insql, $params] = $DB->get_in_or_equal(array_values($attemptids), SQL_PARAMS_NAMED);
    ...
    private function get_record_list(
        ...
        foreach ($bestattemptpescale as $scale => $attempt) {
            $recordlist[] = [
              ...
              'scale' => format_string($scalemap[$scale]['name'], true, ['context' => \context_system::instance(), 'filter' => false]),
              'link' =>
     ...
     */
    private function check_scale_best_attempt(
        $scale,
        &$bestpartialpassedrecords,
        $record,
        &$bestpartialpassedattemptids,
